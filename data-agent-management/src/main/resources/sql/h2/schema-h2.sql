...
CREATE TABLE IF NOT EXISTS harness_skill (
  ...
  update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  CONSTRAINT uk_harness_skill_name UNIQUE (skill_name),
...
CREATE TABLE IF NOT EXISTS harness_skill_resource (
  ...
  update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  CONSTRAINT uk_harness_skill_resource UNIQUE (skill_id, resource_path),
...
CREATE TABLE IF NOT EXISTS report_artifact (
  ...
  content TEXT NOT NULL COMMENT 'Markdown报告内容',
  create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS knowledge_parse_chunk (
  ...
  FOREIGN KEY (revision_id) REFERENCES knowledge_parse_revision (id) ON DELETE CASCADE
) ENGINE=InnoDB;
