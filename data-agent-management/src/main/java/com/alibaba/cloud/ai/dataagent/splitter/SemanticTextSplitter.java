...
public class SemanticTextSplitter extends TextSplitter {
	...
	public List<String> splitText(String text) {
		...
		// 4. 计算 Embeddings
		List<double[]> embeddings = batchEmbed(contextSentences);
	 ...
	 * 核心逻辑：合并句子
	 */
	private List<String> combineSentences(List<String> sentences, List<double[]> embeddings) {
	...
	private List<double[]> batchEmbed(List<String> texts) {
		...
		for (int i = 0; i < texts.size(); i += embeddingBatchSize) {
			...
			try {
				allEmbeddings.addAll(embeddingClient.embedAll(batch));
			}
			catch (Exception e) {
