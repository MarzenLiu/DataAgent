...
import type { AgentStreamRequest, AgentToolActivity } from '../graph';
...
export interface SessionRuntimeState {
	...
	/** 最后一次请求参数 */
	lastRequest: AgentStreamRequest | null;
	/** 当前 Agent 文本输出 */
...
export function useSessionStateManager() {
	...
	const syncStateToView = (
		...
		viewState: {
			...
			toolActivities: Ref<AgentToolActivity[]>;
		},
	) => {
		const state = getSessionState(sessionId);
		viewState.isStreaming.value = state.isStreaming;
		viewState.toolActivities.value = state.toolActivities;
	};
	...
	const saveViewToState = (
		...
		state.toolActivities = viewState.toolActivities.value;
	};
